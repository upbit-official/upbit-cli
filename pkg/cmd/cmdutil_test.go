...
import (
	...
	upbit "github.com/upbit-official/upbit-sdk-go"
...
func TestDefaultRequestOptionsHeaders(t *testing.T) {
	...
	cmd := &cli.Command{Name: "accounts list"}
	...
	_, _ = client.TradingPairs.List(t.Context(), upbit.TradingPairListParams{})

	tests := []struct {
		header string
		want   string
	}{
		{"X-Upbit-Client-Type", "cli"},
		{"X-Upbit-Client-Name", "upbit-cli"},
	}

	for _, tc := range tests {
		got := captured.Get(tc.header)
		assert.Equal(t, tc.want, got, "header %s", tc.header)
	}

	assert.NotEmpty(t, captured.Get("X-Upbit-Client-Version"), "X-Upbit-Client-Version should be set")
}
